<aside class="w-[276px] max-w-[276px] bg-white border-r border-slate-200/70 flex flex-col z-50 h-screen overflow-hidden shadow-sm">
    ...
    <nav class="flex-1 px-4 pb-10 overflow-y-auto no-scrollbar">
        ...
        <div x-data="{ open: {{ request()->routeIs('dashboard.pendeta.*') || request()->routeIs('dashboard.penatua.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-collapse x-cloak class="pl-12 flex flex-col gap-1 mb-2">
                ...
                <a href="{{ route('about.pendeta') }}" target="_blank" class="text-xs font-medium py-2 text-gray-400 hover:text-primary flex items-center gap-1.5">
                    <span>Halaman Pendeta (Visitor)</span>
                ...
                <a href="{{ route('about.penatua') }}" target="_blank" class="text-xs font-medium py-2 text-gray-400 hover:text-primary flex items-center gap-1.5">
                    <span>Halaman Penatua (Visitor)</span>
                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                </a>
            </div>
        </div>
    ...
    <div class="p-6 border-t border-gray-50 flex flex-col gap-2">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full flex items-center gap-3 px-4 py-2 text-rose-500 font-bold text-sm hover:bg-rose-50 rounded-xl transition-all">
