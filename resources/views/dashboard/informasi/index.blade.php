        ...
        <table class="w-full text-left">
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                    ...
                    <td class="px-8 py-5">
                        <a href="{{ route('dashboard.informasi.show', $b->id) }}" class="font-bold text-gray-700 block text-base hover:text-primary transition-colors">{{ $b->judul }}</a>
                        <span class="text-[11px] text-gray-400 font-medium uppercase tracking-widest">{{ $b->kategori ?? 'Informasi' }}</span>
                    </td>
                    <td class="px-8 py-5">
                        @if($b->gambar)
                    ...
                    <td class="px-8 py-5 text-right">
                        <div class="flex items-center justify-end gap-3">
                            <a href="{{ route('dashboard.informasi.show', $b->id) }}" class="px-4 py-2 bg-white border border-gray-100 rounded-lg text-xs font-bold text-gray-600 hover:bg-gray-50 hover:text-primary transition-all shadow-sm">Detail</a>
                            <a href="{{ route('dashboard.informasi.edit', $b->id) }}" class="px-4 py-2 bg-primary text-white rounded-lg text-xs font-bold shadow-lg shadow-primary/10 hover:bg-blue-700 transition-all">Ubah</a>
                            <form action="{{ route('dashboard.informasi.destroy', $b->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus informasi ini?')">
                                @csrf
                                @method('DELETE')
