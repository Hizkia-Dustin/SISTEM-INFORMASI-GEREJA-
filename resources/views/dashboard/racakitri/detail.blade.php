...
<div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden max-w-4xl">
    ...
    <div class="p-12">
        <div class="flex items-center gap-4 mb-6">
            ...
            <span class="text-xs text-gray-400 font-bold tracking-widest">{{ $racakitri->created_at->format('d F Y') }}</span>
        </div>
        ...
        <div class="mt-12 pt-8 border-t border-gray-50 flex items-center gap-4">
            ...
            <form action="{{ route('dashboard.racakitri.destroy', $racakitri->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus racakitri ini?')">
                @csrf
                @method('DELETE')
