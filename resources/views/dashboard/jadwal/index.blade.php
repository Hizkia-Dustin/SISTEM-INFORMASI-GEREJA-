        ...
        <table class="w-full text-left">
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td class="px-8 py-4 font-bold text-gray-800">{{ $j->nama ?? $j->nama_acara ?? '-' }}</td>
                    <td class="px-8 py-4 text-gray-600">{{ $j->tanggal ? \Carbon\Carbon::parse($j->tanggal)->format('d M Y') : '-' }}</td>
                    <td class="px-8 py-4 text-gray-600">{{ $j->waktu ? \Carbon\Carbon::parse($j->waktu)->format('H:i') : ($j->waktu_mulai ?? '-') }}</td>
                    <td class="px-8 py-4 text-gray-600">
                        <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-[10px] font-bold tracking-widest uppercase">{{ $j->jenis ?? $j->lokasi ?? '-' }}</span>
                    </td>
                    <td class="px-8 py-4 text-gray-600">
                        {{ $j->jumlah_hadir ?? (preg_match('/Jumlah hadir: ([0-9]+)/', $j->deskripsi ?? '', $match) ? $match[1] : '-') }}
                    </td>
                    <td class="px-8 py-4">
                        @php
                            $lampiran = $j->lampiran;
                            if (empty($lampiran) && !empty($j->deskripsi) && preg_match('/Lampiran: ([^\s]+)/', $j->deskripsi, $match)) {
                                $lampiran = $match[1];
                            }
                        @endphp
                        @if($lampiran)
                            <a href="{{ asset('storage/' . $lampiran) }}" target="_blank" class="text-primary hover:underline text-xs font-bold flex items-center gap-1">
                    ...
                    <td class="px-8 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            ...
                            <form class="confirm-delete" action="{{ route('dashboard.jadwal.destroy', $j->id) }}" method="POST" data-confirm="Apakah Anda yakin ingin menghapus jadwal ibadah ini?">
                                @csrf
                                @method('DELETE')
