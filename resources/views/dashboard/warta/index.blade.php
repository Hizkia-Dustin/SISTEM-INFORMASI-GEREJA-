...
@section('content')
<x-dashboard.page-header title="Daftar Warta Jemaat" subtitle="Informasi kegiatan dan warta terbaru seputar GKI Pakuwon.">
    <a href="{{ route('dashboard.warta.create') }}" class="px-5 py-2.5 bg-primary text-white rounded-xl text-sm font-bold shadow-lg shadow-primary/20 hover:bg-blue-700 transition-all flex items-center gap-2">
        ...
        <table class="w-full text-left">
            <thead class="bg-gray-50/50 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                <tr>
                    ...
                    <th class="px-8 py-4">Judul Warta</th>
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                    ...
                    <td class="px-8 py-5">
                        <a href="{{ route('dashboard.warta.show', $b->id) }}" class="font-bold text-gray-700 block text-base hover:text-primary transition-colors">{{ $b->judul }}</a>
                    ...
                    <td class="px-8 py-5 text-right">
                        <div class="flex items-center justify-end gap-3">
                            <a href="{{ route('dashboard.warta.show', $b->id) }}" class="px-4 py-2 bg-white border border-gray-100 rounded-lg text-xs font-bold text-gray-600 hover:bg-gray-50 hover:text-primary transition-all shadow-sm">Detail</a>
                            <a href="{{ route('dashboard.warta.edit', $b->id) }}" class="px-4 py-2 bg-primary text-white rounded-lg text-xs font-bold shadow-lg shadow-primary/10 hover:bg-blue-700 transition-all">Ubah</a>
                            <form class="confirm-delete" action="{{ route('dashboard.warta.destroy', $b->id) }}" method="POST" data-confirm="Apakah Anda yakin ingin menghapus warta ini?">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-8 py-12 text-center text-gray-400 italic">Belum ada warta terdaftar.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
