                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                    ...
                    <td class="px-8 py-5 text-right">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('dashboard.warta.show', $b->id) }}" class="px-3 py-1.5 bg-white border border-gray-100 rounded-lg text-xs font-bold text-gray-500 hover:text-primary transition-all">Detail</a>
                            <a href="{{ route('dashboard.warta.edit', $b->id) }}" class="text-primary font-bold text-xs hover:underline">Ubah</a>
                            <form action="{{ route('dashboard.warta.destroy', $b->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus warta ini?')">
                                @csrf
                                @method('DELETE')
