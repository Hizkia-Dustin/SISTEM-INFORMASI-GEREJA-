...
<div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden max-w-4xl">
    <div class="h-64 bg-gray-50 flex items-center justify-center overflow-hidden border-b border-gray-50">
        @if($video->gambar)
            <img src="{{ asset('storage/' . $video->gambar) }}" alt="{{ $video->judul }}" class="w-full h-full object-cover">
        @else
    ...
    <div class="p-12">
        <div class="flex items-center gap-4 mb-6">
            <span class="px-3 py-1 bg-blue-50 text-primary text-[10px] font-extrabold rounded uppercase tracking-widest">{{ $video->kategori }}</span>
            <span class="text-xs text-gray-400 font-bold tracking-widest">{{ $video->created_at->format('d F Y') }}</span>
        </div>
        ...
        <div class="mt-12 pt-8 border-t border-gray-50 flex items-center gap-4">
            ...
            <form action="{{ route('dashboard.video.destroy', $video->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus video ini?')">
                @csrf
                @method('DELETE')
